...
// exists check if name is file or dir
...
func CopyFile(src string, dst string) error {
	...
	if err != nil {
		return fmt.Errorf("can't get stat of %s, err: %w", src, err)
	}
	...
	if err != nil {
		return fmt.Errorf("can't open source file %s, err: %w", src, err)
	}
	defer srcFh.Close()

	// make dir with full rwx for user and only read for user's group
	err = os.MkdirAll(filepath.Dir(dst), 0740)
	if err != nil {
		return fmt.Errorf("can't make destination directory %s, err: %w", filepath.Dir(dst), err)
	}

	dstFh, err := os.Create(dst)
	if err != nil {
		return fmt.Errorf("can't create destination file %s, err: %w", dst, err)
	}
	defer dstFh.Close()

	size, err := io.Copy(dstFh, srcFh)
	if err != nil {
		return fmt.Errorf("can't copy data, err: %w", err)
	}
	if size != srcInfo.Size() {
		err = DeleteFile(dst)
		if err != nil {
			return fmt.Errorf("error during deleteing file by reason of incomplete copy copy, err: %w", err)
		}
...
// DeleteFile deletes file if it's regular.
func DeleteFile(tg string) error {
	tgInfo, err := os.Stat(tg)
	if err != nil {
		return fmt.Errorf("can't get stat of %s, err: %w", tg, err)
	}

	if !tgInfo.Mode().IsRegular() {
		return fmt.Errorf("can't delete non-regular source file %s (%s)", tg, tgInfo.Mode().String())
	}

	err = os.Remove(tg)
	if err != nil {
		return fmt.Errorf("can't delete destination file %s, err: %w", tg, err)
	}
...
func DeleteDir(tg string) error {
	tgInfo, err := os.Stat(tg)
	if err != nil {
		return fmt.Errorf("can't get stat of %s, err: %w", tg, err)
	}

	if !tgInfo.Mode().IsDir() {
		return fmt.Errorf("can't delete non-dir source file %s (%s)", tg, tgInfo.Mode().String())
	}

	err = os.RemoveAll(tg)
	if err != nil {
		return fmt.Errorf("error during recursive deleting directory %s, err: %w", tg, err)
	}
