...
D 1st programm on the way to understanding CUDA.
...
__global__ void kernel(float* A, float* b, float* c) {
    ...
    shared_b[i] = b[i];
    __syncthreads();
    elementIndex = blockIdx.x;
    cacheIndex = threadIdx.x;
    cache[cacheIndex] = A[elementIndex*N + cacheIndex]*shared_b[cacheIndex];
    __syncthreads();
    int i = blockDim.x/2;
    while (i != 0) {
        if (cacheIndex < i)
...
int main(int argc, char* argv[]) {
    ...
    /* Log time */
    printf("when vector b is shared and using parallel preduction by blocks:\n")
    printf("time :  %f\n", elapsedTime);
